...
// This example demonstrates how DependencyGraph models dependencies between
// operations as a directed acyclic graph (DAG).
//
// DependencyGraph allows operations to depend on multiple "chains" of causality.
...
func ExampleDependencyGraph() {
	// When using DependencyGraph, users choose the type that identifies the chains
	...
	checkStatus := func(name string, op causalorder.Operation) {
		if Completed(op) {
